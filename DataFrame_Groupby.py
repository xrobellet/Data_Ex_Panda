...
sortRatingsField_Xavier = cloneDF(mergeRatings)
sortRatingsField_Xavier = sortRatingsField_Xavier.groupby(['movie_id', 'title'])['rating'].agg(
    {'COUNT': np.size, 'myAVG': lambda x: x.sum() / float(x.count())}).sort_values('COUNT', ascending=False)
print('My info sorted: \n%s' % sortRatingsField_Xavier[:15])
